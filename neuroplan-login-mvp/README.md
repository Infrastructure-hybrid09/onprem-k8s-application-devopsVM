...
- **AI 문제 생성 안정화:** 문제 생성에는 최대 2,200 출력 토큰을 별도로 허용하고 문제·보기·해설 길이를 제한해 JSON 잘림을 줄입니다. `correct`, `isCorrect`, `correctOptionNo` 응답 형식을 모두 검증해 처리합니다.
- **상단 토큰 게이지:** 오른쪽 위에 `잔여 / 일일 한도` 숫자와 색상 잔량 막대를 함께 표시합니다.
- **기능별 평균 토큰:** 성공하고 환불되지 않은 실제 요청을 기준으로 학습 플랜·AI 문제·오답 해설·재학습 추천의 평균 사용 토큰과 표본 수를 안내합니다.
