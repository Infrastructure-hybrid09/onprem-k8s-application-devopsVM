...
if ([string]::IsNullOrWhiteSpace($ArchivePath)) {
    $ArchivePath = Join-Path -Path $PSScriptRoot -ChildPath "neuroplan-login-mvp-0.8.0-bundle.zip"
}
...
if ($ArchivePath -match '[<>]') {
    throw "ArchivePath contains a placeholder character (< or >). Use the actual archive filename, for example: neuroplan-login-mvp-0.8.0-bundle.zip"
}
...
Write-Host "===== NeuroPlan 0.8.0 transfer ====="
